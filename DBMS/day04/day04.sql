...
SELECT  * FROM PLAYER ORDER BY HEIGHT ASC; // 오름차순이 DEFALUT 
SELECT  * FROM PLAYER ORDER BY HEIGHT ; 
SELECT  * FROM PLAYER ORDER BY HEIGHT DESC; 
SELECT * FROM PLAYER ORDER BY 12 DESC; // 컬럼의 번호를 사용할 수 있다.
...
-- 사원 별 급여가 3000이상이면이상, 5000이하면 '연봉 증가'
...
FIRST_NAME ,
CASE 
	WHEN SALARY BETWEEN 3000 AND 5000 THEN '연봉증가'
	WHEN SALARY BETWEEN 5001 AND 8000 THEN '연봉동결' 
	ELSE '연봉하락'
END "연봉 테이블"
FROM EMPLOYEES;
---------------------------------------------------------------------------
...
-- ※WHERE절에 우선적으로 처리할 조선식을 작성해야 속도가 빠르다. 
...
SELECT "JOB_ID", AVG(SALARY),MAX(SALARY) FROM EMPLOYEES 
GROUP BY JOB_ID
...
-- 다음날 정답 풀이
SELECT SUM(HEIGHT) / COUNT(NVL(HEIGHT, 0)) 전체선수 ,
SUM(CASE WHEN "POSITION" LIKE 'MF' THEN HEIGHT END) /COUNT(CASE WHEN "POSITION" LIKE 'MF' THEN HEIGHT END)"MF 평균 키",
SUM(CASE WHEN "POSITION" LIKE 'FW' THEN HEIGHT END) /COUNT( CASE WHEN "POSITION" LIKE 'FW' THEN HEIGHT END )"FW",
SUM(CASE WHEN "POSITION" LIKE 'DF' THEN HEIGHT END) /COUNT( CASE WHEN "POSITION" LIKE 'DF' THEN HEIGHT END )"DF 평균 키",
SUM(CASE WHEN "POSITION" LIKE 'GK' THEN HEIGHT END) /COUNT( CASE WHEN "POSITION" LIKE 'GK' THEN HEIGHT END )"GK 평균 키"
FROM PLAYER ;

SELECT COUNT(NVL(HEIGHT, 0)) FROM PLAYER;
SELECT COUNT(HEIGHT) FROM PLAYER;

SELECT COUNT(CASE WHEN "POSITION" LIKE 'DF' THEN HEIGHT END) FROM PLAYER; -- COUNT는 컬럼이 NULL이라면 카운트 하지 않는다. 
SELECT COUNT(CASE WHEN "POSITION" LIKE 'DF' THEN NVL(HEIGHT, 1) END) FROM PLAYER; -- NULL이 포함된다. 

SELECT * FROM PLAYER WHERE "POSITION" LIKE 'DF' ;
SELECT * FROM PLAYER WHERE "POSITION" LIKE 'DF' AND HEIGHT IS NOT NULL;

SELECT  CASE WHEN "POSITION" LIKE 'DF' THEN HEIGHT END FROM PLAYER;
