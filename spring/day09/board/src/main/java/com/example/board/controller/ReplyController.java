...
// REST 컨트롤러는 타 서버 간 통신이나 화면이동이 아닌 데이터 이동일때 사용
...
//    ResponseEntity : 서버의 상태코드, 응답 메세지 등을 담을 수 있는 타입
    ...
    public List<ReplyVO> getList(@PathVariable("bno") Long bno, @PathVariable("pageNum") int pageNum){
        log.info("getList.........." + bno + ", " + pageNum);
        return replyService.getList(bno, new Criteria(pageNum, 10));
    }
...
//    - DB에서 디폴트값이 없는 값이 없을떄 NULL이 들어가서 생기는 오류를 옵션을 통해서 막을 수 있다.
...
/* @PathVariable("rno")   @PatchMapping에서 value하나일경우 생략가능 -> @PathVariable()*/
//      value =  {"/{rno}","/{rno}/{replier}"}
//      ↑외부에서 데이터가 전달될 떄 일부분만 전달하면 그 에 따른 URI를 위와 같이 구분해준다.
//      ReplyVO에 전달될 JSON 데이터 중 replier가 전달되지 않았을 경우 Validate, Validated, Valid 등을 사용하여 검증 단계를 거쳐야 한다,
//      하지만 복작하고, 이걸 쓸빠에 POST를 쓰기 때문에 현장에서 잘 사용하지 않는다.
...
//    public String modify(@RequestBody ReplyVO replyVO, @PathVariable(value = "replier", required = false) String replier,@PathVariable("rno") Long rno){
//        /* consumes에서 매개변수로 자동주입이 되지 않기 때문에 @RequestBody를 사용
//        *   required를 사용하여 예외처리를 발생시키지않고 null받을수 있게함
//        *
//        *  */
//        log.info("modify.............." +replyVO);
//        if(replyVO.getReplier() == null){ // JSON 검증 -> 더 높은 단계에서는 JsonMergePatch ,Validate 처리
