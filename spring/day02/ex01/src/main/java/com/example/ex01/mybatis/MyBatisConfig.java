...
// was 가 돌아갈떄 참조하라고 어노테이션을 사용해야함
// 주입 x
@Configuration
@RequiredArgsConstructor
@MapperScan("com.example.ex01.mappers") // 매퍼를 연결
public class MyBatisConfig {
// 커넥션 풀 및 MyBatis에 필요한 요소를 메모리에 할당 및 관리, xml과 java연동에 필요한 경로 관리
    private final ApplicationContext applicationContext;


    // 메소드위에 Bean 사용시 메소드를 스프링 컨테아너에 등록
    @Bean // 매소드의 리턴 객체를 스프링 컨테이너에 등록 @Configuration 또는 @Component가 작성된 클래스의 메소드에 사용
    // application.properties에 작성된 #JDBC datasource 정보 설정
    @ConfigurationProperties(prefix = "spring.datasource.hikari") // 상위 경로 고정 // 컨피그를 들렸다 온다
...
//    dataSource 객체에 DBMS 정보 설정
    ...
    // 위의 작업으로 SqlSessionFactory 빌더 까지 작업이 된 것
    @Bean
