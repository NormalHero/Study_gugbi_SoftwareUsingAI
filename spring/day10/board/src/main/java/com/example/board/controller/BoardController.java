...
//        이 때 같은 컨트롤러라면, 전체 URL 경로가 아닌 메소드의 Mapping URL만 작성해준다.
        return new RedirectView("list"); // 핸들러 매핑으로 가기떄문에 perfix가 자동으로 붙는다
    }
    ...
    public RedirectView remove(Long bno, Criteria criteria,RedirectAttributes rttr){
        ...
        // addFlashAttribute는 컨트롤러를 거치지 않음
        // 컨트롤러를 거치기 위해서는 addAttribute(쿼리스트링)으로 보내야한다
        // Redirect로 전송 시
        // addAttribute()를 사용하면 컨트롤러에 파라미터가 전달되고 그걸 통해서 화면으로 간다.
        // addFlashAttribute()를 사용하면 컨트롤러에 전달도지 않고 바로 화면으로 간다.
...
//        return "/practice/boardPractice";
//    }

//    @PostMapping("/writeBoardPractice")
//    public String writeBoardPractice(BoardVO boardVO){
//        log.info("--------------------------------------");
//        log.info(boardVO.toString());
//        log.info("--------------------------------------");
//        boardService.register(boardVO);
//        return "forward:listPractice"; // 요청값을 보낸다 -> 요청과 메소드명을 통일하자
//    }
//    @GetMapping("/writeBoardPractice")
//    public String writeBoardPractice(){
//        log.info("들어옴,ㅁㄴ이,ㅁㅇ");
//        return "/practice/boardWritePractice";
//    }
